#2019, Hannah Raphaëlle Petitclerc-Vilandré
#Coding : utf-8
...
### Tout d'abord, tu m'as joué un tour en créant un autre répertoire avec le même nom... Je ne trouvais pas ton script, lundi matin.
### Mais j'ai fini par le retrouver. Et je constate qu'il n'était pas en retard. :)

### Ce n'est pas une faute, mais voici autre façon d'importer une longue variable.
### Je n'ai pas eu le temps de vous la montrer la semaine dernière.
### Il s'agit de mettre la variable «articles» dans un fichier à part.
### Ici, c'est le fichier eureka.py, qui se trouve aussi dans ton répertoire.
### C'est un fichier python qui ne contient que la déclaration de la variable «articles»
### Il suffit de l'importer avec le code suivant:
...
### Très bien comme vérifications:
print(articles[0]) #je vérifie que c'est bien l'indexe du premier article de Lapresse+
print(articles[-1])#Je vérifie que la line 614 est bien l'indexe -1. 

for article in articles:
	print()
	article = article.split(",") # Je sépare ainsi chaque ligne de charque article en sections différentes avec donc 4 indexes (0,1,2,-1)

	### Très bien: mais dans la ligne ci-dessous, comme dans les autres semblables,
	### il n'est pas nécessaire d'ajouter «[0:-1]» à la variable «article»
	### ni de mettre des parenthèses. «article[0]» suffit pour isoler le nom du média.
	### En ajoutant un «.strip()» à la fin, pour être sûr.
	media = (article[0:-1][0]) # Correspond au premier mot avant la "," de la première ligne à la dernière ligne donc le nom de chaque Média.
	print(media)

	section = (article[0:-1][1]) # Correspond au deuxième mot après la première "," de la première ligne à la dernière ligne donc aux noms des différentes sections de chaque média.
	print(section)

	motsdate = (article[0:-1][2])
	print(motsdate)
	mots = motsdate.replace("samedi 26 janvier 2019"," ") # puisque la troisième série de mots correspond à la date ET au nombre de mots on remplace la date par du "vide" pour ne voir au final que le nombre de mots que contiennent les articles. 
	print(mots)
	...
	print(page)
	### C'est à ta ligne 629 qu'il fallait simplement changer le «.split(",")» par «.split(", ")»
...
### C'est un bel effort, d'autant plus que je ne vous ai pas parlé de la méthode «.count()»
### Mais la meilleure façon était d'utiliser un compteur. Tu crées 6 variables (une pour chaque média),
### puis tu ajoutes 1 au compteur correspondant au Devoir, par exemple, si la variable «media» est égale à «Le Devoir».
...
### Mais c'est un très bon script, au final.
### En terminant, félicitations pour le soin que tu as apporté aux commentaires!